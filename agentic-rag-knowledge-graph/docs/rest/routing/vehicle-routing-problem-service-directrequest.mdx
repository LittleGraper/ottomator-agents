...
uid: "vehicle-routing-problem-direct"
slug: "vehicle-routing-problem-service-directrequest"
url: "/routing/vehicle-routing-problem-service-directrequest"
guid: "GUID-F321EF42-6958-4DB6-A5F4-0F6F3DE82772"
migratedTopicMetadata:
  FTITLE: "Vehicle Routing Problem Service_DirectRequest"
  FDESCRIPTION: ""
  ...
  CREATED-ON: "21/02/2024 11:52:18"
  FAUTHOR: "pchopra"
  FSTATUS: "Released"
  MODIFIED-ON: "04/03/2024 11:54:41"
  VERSION: "7"
  FRESOLUTION: ""
  DOC-LANGUAGE: "en"
  FISHREVCOUNTER: "2"
  ED: "GUID-BB908C9B-B6DA-4911-BCAB-CB3F8014F57D"
...
To learn more about the job request, see [Job request](/vehicle-routing-problem-service) documentation. 

You can make a direct request to the fleet routing service using the following form:
...
Specify whether orders assigned to an individual route are spatially clustered. 
...
Specify whether hierarchy should be used when finding the best routes for the fleet of vehicles. 
...
You can specify the value of this parameter as in the following example:
...
Specify whether the service should generate driving directions for the routes that are included as part of the fleet routing problem solution. 
...
Specify whether the tool should create the shapes for the output assigned and unassigned stops. 
...
[locate_settings](/vehicle-routing-problem-service-directrequest#locate_settings) 
...
A maximum of 100 orders can be specified for direct request.
...
-   `CurbApproach` 
    ...
    When the vehicle approaches the order, the order can be on either side of the vehicle; however, when it departs, the vehicle must continue in the same direction in which it arrived. A U-turn is prohibited.
...
Example one: Specifying orders in the spatial reference of the network dataset using JSON structure. The example also shows how to specify attributes for the orders.
...
![U-turns permitted only at intersections and dead-ends](../../shared/routing/images/U_TurnsAtJunctions_IntersectAndDeadEnds.png)
...
-   `IsHardZone` 
    ...
    Specifies whether the route zone is hard or soft. The attribute value is specified as an integer with possible values being `0`  or `1` . A value of `1`  indicates that the route zone is hard; that is, an order that falls outside of the route zone area can't be assigned to the route. A value of `0`  indicates that such orders can still be assigned, but the cost of servicing the order is weighted by a function that is based on the Euclidean distance from the route zone. Basically, this means that as the straight-line distance from the soft zone to the order increases, the likelihood of the order being assigned to the route decreases.
...
The parameter value can be specified as a well-known ID (WKID) for the spatial reference. See [Using spatial references](DEVSITE/services-reference/enterprise/using-spatial-references) to look up WKID values.
...
[Learn more about locating inputs](https://pro.arcgis.com/en/pro-app/latest/help/analysis/networks/locating-analysis-inputs)
...
The type of request made to the service determines the way in which you can retrieve the results from the service.

### Outputs from direct request
...
Upon successful running of the service, it returns the solution to the specified fleet routing problem. The solution is made up of the following output parameters: 
...
The `out_stops`  parameter is returned as a JSON feature set with the following syntax:
...
The `out_routes`  parameter is returned as a JSON feature set with the following syntax:
...
The service can only return a maximum of 10,000 features when using the direct request. If this limit is exceeded, the `exceededTransferLimit`  property is set to `true` . 
...
The `out_unassigned_stops`  parameter is returned as a JSON feature set with the following syntax.
