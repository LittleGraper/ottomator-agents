...
description: "Learn how to use the fleet routing problem service available with ArcGIS Location Services."
uid: "vehicle-routing-problem-job"
slug: "vehicle-routing-problem-service"
url: "/routing/vehicle-routing-problem-service"
guid: "GUID-72B7F548-0777-441F-90BF-0A50B5A7AD34"
migratedTopicMetadata:
  FTITLE: "Vehicle Routing Problem Service"
  FDESCRIPTION: ""
  FCHANGES: "Fix caution note in route line simplification tolerance param"
  FISHRELEASELABEL: ""
  FESRISOFTWARERELEASE: ""
  FESRITAGCLOUD: ""
  FESRIRECREATIONPROCEDURE: ""
  FESRIPRODUCTLIFECYCLE: "agol Feb2024 - *"
  CREATED-ON: "21/02/2024 11:51:01"
  FAUTHOR: "pchopra"
  FSTATUS: "Released"
  MODIFIED-ON: "04/03/2024 11:54:39"
  VERSION: "29"
  FRESOLUTION: ""
  DOC-LANGUAGE: "en"
  FISHREVCOUNTER: "2"
  ED: "GUID-B044179D-7A26-48A9-880C-C8E1764ED21C"
...
See the [tutorial](https://developers.arcgis.com/labs/rest/get-optimized-routes-for-multiple-vehicles/) to learn more about how to solve the multiple fleet routing problem.

## Request URL

The Fleet routing service supports [direct request](DEVSITE/documentation/glossary/direct-request/) and [job request](DEVSITE/documentation/glossary/job-request/). You can choose a request type based on the size of the problem you are trying to solve.

To learn more about the direct request, see [Direct request](/vehicle-routing-problem-service-directrequest) documentation. 

You can make a job request to the fleet routing service using the following form:
...
The fleet routing problem request takes the parameters listed in the subsections below. The only required parameters are `orders` , `depots` , `routes` , `token` , and `f` . The optional parameters have default values that will be used if the parameter is not specified in the request.
...
[depot](/vehicle-routing-problem-service#depots)
...
Specify the routes that are available for the given fleet routing problem.
...
Provide the identity of a user who has the permissions to access the service.
...
Specify the date on which the routes start. The parameter value should be specified as a numeric value representing the milliseconds since midnight, January 1, 1970.
...
Specify the rest periods, or breaks, for the routes in a given fleet routing problem.
...
[See the list](/vehicle-routing-problem-service#analysis_region) of acceptable values.
...
Specify if hierarchy should be used when finding the best routes for the fleet of vehicles. 
...
To view all accepted values for this parameter, see the [restrictions](/vehicle-routing-problem-service#restrictions)  section below.
...
Specify the amount by which you want to simplify the route geometry returned by the service. 
...
Specify if the service should generate driving directions for the routes that are included as part of the fleet routing problem solution. 
...
Specify the language that should be used when generating driving directions. Applies only when the `populate_directions`  parameter is set to `true` . 

The default value is `en` .

To view all accepted values, see the [directions_language](/vehicle-routing-problem-service#directions_language)  section below.
...
Specify the name of the formatting style for the directions. Applies only when the `populate_directions`  parameter is set to `true` . 
...
Specify whether the service should create a `.zip`  file that contains a file geodatabase holding the inputs and outputs of the analysis in a format that can be used to share route layers with your portal. 
...
Specify if the service should save the analysis settings as a network analysis layer file. 
...
Specifies whether invalid orders will be ignored when solving the fleet routing problem. 
...
A maximum of 2,000 orders can be specified for a job request. 

The `orders`  parameter can be specified using a JSON structure that represents a set of point features. The property is optional; however, the JSON structure must specify either the `url`  or `features`  property: 
...
-   `InboundArriveTime` 
    ...
    The value for the `InboundArriveTime`  attribute is specified as an integer that represents the number of milliseconds since epoch (January 1, 1970). 
...
-   `OutboundDepartTime` 
    ...
    The value for the `OutboundDepartTime`  attribute is specified as an integer that represents the number of milliseconds since epoch (January 1, 1970). 
...
The following is the syntax for specifying orders using JSON structure for features:
...
The following is the syntax for specifying orders using a URL returning a JSON response:
...
Example one: Specifying orders in the spatial reference of the network dataset using JSON structure. The example also shows to specify attributes for the orders.
...
The `depots`  parameter can be specified using a JSON structure that represents a set of point features. The property is optional; however, the JSON structure must specify either the `url`  or `features`  property: 
...
The following is the syntax for specifying depots using JSON structure for features:
...
The following is the syntax for specifying depots using a URL returning a JSON response:
...
Example one: Specifying depots in the spatial reference of the network dataset using JSON structure. The example shows how to specify some attributes for the depots. 
...
The `routes`  parameter can be specified using a JSON structure that represents a set of point features. The property is optional; however, the JSON structure must specify either the `url`  or `features`  property: 
...
The following is the syntax for specifying routes using JSON structure for features:
...
The following is the syntax for specifying routes using a URL returning a JSON response:
...
The following is the syntax for specifying breaks using JSON structure for features:
...
The following is the syntax for specifying breaks using a URL returning a JSON response:
...
This example shows how to specify travel-time-based breaks by specifying the value for the `MaxTravelTimeBetweenBreaks`  attribute. The input fleet routing problem requires that the driver for each of the two routes have one 15-minute unpaid break after two hours of driving and another unpaid break after an additional two hours of driving.
...
This example shows how to specify work-time-based breaks by specifying the value for the `MaxCumulWorkTime`  attribute. The input fleet routing problem requires that the driver for each of the two routes have one 15-minute paid break after two hours of work time and a second 15-minute paid break after three hours of work time since the first break.
...
The default value for this parameter is `ALLOW_DEAD_ENDS_AND_INTERSECTIONS_ONLY` .
...
![Low importance](../../shared/routing/images/VRP_TimeWindows_LowImportance.png)
...
![High importance](../../shared/routing/images/VRP_TimeWindows_HighImportance.png)
...
The following is the syntax for specifying route zones using JSON structure for features:
...
The following is the syntax for specifying route zones using a URL returning a JSON response:
...
The following is the syntax for specifying route renewals using JSON structure for features:
...
The following is the syntax for specifying route renewals using a URL returning a JSON response:
...
The `orders_pairs`  parameter can be specified using a JSON structure that represents a set of point features. The property is optional; however, the JSON structure must specify either the `url`  or `features`  property: 

-   `url` —Specify a REST query request to any ArcGIS Server feature, map, or geoprocessing service that returns a JSON feature set. 
-   `features` —Specify an array of features. 

    Each feature in the `features`  array represents an order pair stop and contains the following properties:
...
The following is the syntax for specifying order pairs using JSON structure for features:
...
The following is the syntax for specifying order pairs using a URL returning a JSON response:
...
![Calculating excess transit time](../../shared/routing/images/VRP_penaltytransittime.png)
...
![Low importance](../../shared/routing/images/VRP_ExcessTravelTime_LowImportance.png)
...
![High importance](../../shared/routing/images/VRP_ExcessTravelTime_HighImportance.png)
...
The added travel time when the barrier is traversed. This field is applicable only for added-cost barriers and when the `impedance`  parameter or the `travel_mode`  parameter is time-based.

This field value must be greater than or equal to zero, and its units are the same as those specified in the `time_units`  parameter.

`Additional_Distance` 

The added distance when the barrier is traversed. This field is applicable only for added-cost barriers and when the `impedance`  parameter or the `travel_mode`  parameter is distance-based.

The field value must be greater than or equal to zero, and its units are the same as those specified in the `distance_units`  parameter.
...
-   `ScaledTimeFactor` 
    ...
    This field is applicable only for scaled-cost barriers and when the `impedance`  parameter or the `travel_mode`  parameter is time-based.
-   `ScaledDistanceFactor` 
    ...
    This field is applicable only for scaled-cost barriers and when the `impedance`  parameter or the `travel_mode`  parameter is distance-based.
-   `ScaledCostFactor` 
...
Since fleet routing service only supports time based impedance, the value for 
...
`impedance`
  ...
  parameter must be identical. Otherwise, the service will return an error.
...
`https://logistics.arcgis.com/arcgis/rest/services/World/VehicleRoutingProblem/GPServer/SolveVehicleRoutingProblem/jobs/<yourJobID>/results?token=<yourToken>&f=json` 

Refer to the [Output parameters](/vehicle-routing-problem-service#output-parameters) section for more information on how to interpret the solution provided by the service.
...
`https://logistics.arcgis.com/arcgis/rest/services/World/VehicleRoutingProblem/GPServer/SolveVehicleRoutingProblem/jobs/<yourJobID>/results/<output_parameter_name>?token=<yourToken>&f=json&outSR=102100` 
...
`https://logistics.arcgis.com/arcgis/rest/services/World/VehicleRoutingProblem/GPServer/SolveVehicleRoutingProblem/jobs/<yourJobID>/results?token=<yourToken>&f=json&outSR=102100` 
...
`https://logistics.arcgis.com/arcgis/rest/services/World/VehicleRoutingProblem/GPServer/SolveVehicleRoutingProblem/jobs/<yourJobID>/inputs?token=<yourToken>&f=json` 

## Output parameters

Upon successful execution, the service returns the solution to the specified fleet routing problem. The solution is made up of the following output parameters: 
...
The `ObjectID`  value of the input order, depot, or break. This field can be used to join attributes from input features. 
...
Unlike the `out_routes`  parameter, the `out_stops`  parameter does not contain `spatialReference`  or `geometryType`  properties because this parameter does not return any geometry information about the stops.
...
These fields contain a summary of violated constraints and are set after a solve service. Each field will contain one violation. If an order has more than one violation the next `ViolatedConstraint_#`  field will be used.
...
When using the job request, the service can only return a maximum of 200,000 features with the `out_directions`  parameter. If this limit is exceeded, the `exceededTransferLimit`  property is set to `true` . 
...
Use this parameter to determine if the service was able to successfully find the solution for the input fleet routing problem. The error messages for the failure can be obtained by making a request to get the status of the job. 
...
The table below lists the limits that apply to this service. Some limits vary by the type of request, and they are noted accordingly. 
...
Maximum time a client can use the fleet routing services
...
```other
https://logistics.arcgis.com/arcgis/rest/services/World/VehicleRoutingProblem/GPServer/SolveVehicleRoutingProblem/submitJob?
...
"CostPerUnitDistance":1.5,"MaxOrderCount":2,"MaxTotalTime":360,"MaxTotalTravelTime":120,"MaxTotalDistance":80}}]}&time_units=Minutes&distance_units=Miles&uturn_policy=NO_UTURNS&populate_directions=true&directions_language=en&default_date=1355212800000&f=json&token=<yourToken>
```

 

```javascript
https://logistics.arcgis.com/arcgis/rest/services/World/VehicleRoutingProblem/GPServer/SolveVehicleRoutingProblem/submitJob?orders=%7B%22features%22:
%5B%7B%22geometry%22:%7B%22x%22:-122.51,%22y%22:37.7724%7D,%22attributes%22:%7B%22DeliveryQuantities%22:1706,%22Name%22:%22Store_1%22,%22ServiceTime%22:25,
%22TimeWindowStart1%22:1355245200000,%22TimeWindowEnd1%22:1355274000000,%22MaxViolationTime1%22:0%7D%7D,%7B%22geometry%22:%7B%22x%22:-122.4889,%22y%22:37.7538%7D,
%22attributes%22:%7B%22DeliveryQuantities%22:1533,%22Name%22:%22Store_2%22,%22ServiceTime%22:23,%22TimeWindowStart1%22:1355245200000,%22TimeWindowEnd1
%22:1355274000000,%22MaxViolationTime1%22:0%7D%7D,%7B%22geometry%22:%7B%22x%22:-122.4649,%22y%22:37.7747%7D,%22attributes%22:%7B%22DeliveryQuantities%22:1580,
%22Name%22:%22Store_3%22,%22ServiceTime%22:24,%22TimeWindowStart1%22:1355245200000,%22TimeWindowEnd1%22:1355274000000,%22MaxViolationTime1%22:0%7D%7D,%7B%22geometry
%22:%7B%22x%22:-122.4739,%22y%22:37.7432%7D,%22attributes%22:%7B%22DeliveryQuantities%22:1289,%22Name%22:%22Store_4%22,%22ServiceTime%22:20,%22TimeWindowStart1
%22:1355245200000,%22TimeWindowEnd1%22:1355274000000,%22MaxViolationTime1%22:0%7D%7D,%7B%22geometry%22:%7B%22x%22:-122.4493,%22y%22:37.7315%7D,%22attributes
%22:%7B%22DeliveryQuantities%22:1302,%22Name%22:%22Store_5%22,%22ServiceTime%22:21,%22TimeWindowStart1%22:1355245200000,%22TimeWindowEnd1%22:1355274000000,
%22MaxViolationTime1%22:0%7D%7D,%7B%22geometry%22:%7B%22x%22:-122.4917,%22y%22:37.6493%7D,%22attributes%22:%7B%22DeliveryQuantities%22:1775,%22Name%22:%22Store_6%22,
%22ServiceTime%22:26,%22TimeWindowStart1%22:1355245200000,%22TimeWindowEnd1%22:1355274000000,%22MaxViolationTime1%22:0%7D%7D,%7B%22geometry%22:%7B%22x%22:-122.4832,
%22y%22:37.7012%7D,%22attributes%22:%7B%22DeliveryQuantities%22:1014,%22Name%22:%22Store_7%22,%22ServiceTime%22:17,%22TimeWindowStart1%22:1355245200000,
%22TimeWindowEnd1%22:1355274000000,%22MaxViolationTime1%22:0%7D%7D,%7B%22geometry%22:%7B%22x%22:-122.5301,%22y%22:37.8935%7D,%22attributes%22:%7B%22DeliveryQuantities
%22:1761,%22Name%22:%22Store_8%22,%22ServiceTime%22:26,%22TimeWindowStart1%22:1355245200000,%22TimeWindowEnd1%22:1355274000000,%22MaxViolationTime1%22:0%7D%7D,%7B
%22geometry%22:%7B%22x%22:-122.2875,%22y%22:37.8909%7D,%22attributes%22:%7B%22DeliveryQuantities%22:1815,%22Name%22:%22Store_9%22,%22ServiceTime%22:27,
%22TimeWindowStart1%22:1355245200000,%22TimeWindowEnd1%22:1355274000000,%22MaxViolationTime1%22:0%7D%7D%5D%7D&depots=%7B%22features%22:%5B%7B%22geometry%22:
%7B%22x%22:-122.3943,%22y%22:37.7967%7D,%22attributes%22:%7B%22Name%22:%22San%20Francisco%22,%22TimeWindowStart1%22:1355241600000,%22TimeWindowEnd1%22:1355274000000
%7D%7D%5D%7D&routes=%7B%22features%22:%5B%7B%22attributes%22:%7B%22Name%22:%22Truck_1%22,%22StartDepotName%22:%22San%20Francisco%22,%22EndDepotName%22:
%22San%20Francisco%22,%22StartDepotServiceTime%22:60,%22EarliestStartTime%22:1355241600000,%22LatestStartTime%22:1355241600000,%22Capacities%22:%2215000%22,
%22CostPerUnitTime%22:0.2,%22CostPerUnitDistance%22:1.5,%22MaxOrderCount%22:2,%22MaxTotalTime%22:360,%22MaxTotalTravelTime%22:120,%22MaxTotalDistance%22:80%7D%7D,
%7B%22attributes%22:%7B%22Name%22:%22Truck_2%22,%22StartDepotName%22:%22San%20Francisco%22,%22EndDepotName%22:%22San%20Francisco%22,%22StartDepotServiceTime%22:60,
%22EarliestStartTime%22:1355241600000,%22LatestStartTime%22:1355241600000,%22Capacities%22:%2215000%22,%22CostPerUnitTime%22:0.2,%22CostPerUnitDistance%22:1.5,
%22MaxOrderCount%22:2,%22MaxTotalTime%22:360,%22MaxTotalTravelTime%22:120,%22MaxTotalDistance%22:80%7D%7D,%7B%22attributes%22:%7B%22Name%22:%22Truck_3%22,
%22StartDepotName%22:%22San%20Francisco%22,%22EndDepotName%22:%22San%20Francisco%22,%22StartDepotServiceTime%22:60,%22EarliestStartTime%22:1355241600000,
%22LatestStartTime%22:1355241600000,%22Capacities%22:%2215000%22,%22CostPerUnitTime%22:0.2,%22CostPerUnitDistance%22:1.5,%22MaxOrderCount%22:2,%22MaxTotalTime%22:360,
%22MaxTotalTravelTime%22:120,%22MaxTotalDistance%22:80%7D%7D%5D%7D&time_units=Minutes&distance_units=Miles&uturn_policy=NO_UTURNS&populate_directions=true&directions_language=en&default_date=1355212800000&f=json&token=<yourToken>
```
...
```other
https://logistics.arcgis.com/arcgis/rest/services/World/VehicleRoutingProblem/GPServer/SolveVehicleRoutingProblem/jobs/<yourJobID>?
returnMessages=true&f=json&token=<yourToken>
...
```other
https://logistics.arcgis.com/arcgis/rest/services/World/VehicleRoutingProblem/GPServer/SolveVehicleRoutingProblem/jobs/<yourJobID>/results/out_routes?
f=json&token=<yourToken>
```

 

```javascript
https://logistics.arcgis.com/arcgis/rest/services/World/VehicleRoutingProblem/GPServer/SolveVehicleRoutingProblem/jobs/<yourJobID>/results/
out_routes?f=json&token=<yourToken>
```
...
```other
https://logistics.arcgis.com/arcgis/rest/services/World/VehicleRoutingProblem/GPServer/SolveVehicleRoutingProblem/jobs/<yourJobID>/results/
out_stops?f=json&token=<yourToken>
