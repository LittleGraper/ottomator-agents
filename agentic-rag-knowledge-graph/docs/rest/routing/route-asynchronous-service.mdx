...
uid: "route-job"
slug: "route-asynchronous-service"
url: "/routing/route-asynchronous-service"
guid: "GUID-7F53EB26-FF17-407E-831D-02843FB599B6"
migratedTopicMetadata:
  FTITLE: "Route Asynchronous Service"
  FDESCRIPTION: ""
  ...
  CREATED-ON: "28/03/2023 10:03:48"
  FAUTHOR: "pchopra"
  FSTATUS: "Released"
  MODIFIED-ON: "17/04/2023 11:28:10"
  VERSION: "20"
  FRESOLUTION: ""
  DOC-LANGUAGE: "en"
  FISHREVCOUNTER: "3"
  ED: "GUID-032675B7-66E1-4D1F-827D-AAC26F1596B5"
...
The Route service supports [direct request](DEVSITE/documentation/glossary/direct-request/) and [job request](DEVSITE/documentation/glossary/job-request/). You can choose a request type based on the size of the problem you are trying to solve.

To learn more about the direct request, see [Direct request](/route-synchronous-service) documentation. 

You can make a job request to the route service using the following form:

`https://logistics.arcgis.com/arcgis/rest/services/World/Route/GPServer/FindRoutes/submitJob?parameters` 

## Request parameters

The route request takes the required and optional parameters listed below. You always need to include values for the required parameters in your requests. The optional parameters have default values that are used when not specified in the request.
...
[stops](/route-asynchronous-service#stops)
...
-   [As a collection of features](/route-asynchronous-service#syntax-examples-for-stops)
-   [As a feature service URL](/route-asynchronous-service#syntax-examples-for-stops)
...
[token](/route-asynchronous-service#token)
...
[f](/route-asynchronous-service#f)
...
[travel_mode](/route-asynchronous-service#travel_mode)
...
[measurement_units](/route-asynchronous-service#measurement_units)
...
Specify the units that will be used to report the total travel time or travel distance for the output routes. 

The default value is `Minutes` .

Value: `Minutes`  \| `Meters`  \| `Kilometers`  \| `Feet`  \| `Yards`  \| `Miles`  \| `NauticalMiles`  \| `Seconds`  \| `Hours`  \| `Days` 
...
[analysis_region](/route-asynchronous-service#analysis_region)
...
Value: [see list](/route-asynchronous-service#analysis_region)
...
[reorder_stops_to_find_optimal_routes](/route-asynchronous-service#reorder_stops_to_find_optimal_routes)
...
[preserve_terminal_stops](/route-asynchronous-service#preserve_terminal_stops)
...
Specify whether the service will preserve the first and last stops when reordering stops. This parameter is required only when `reorder_stops_to_find_optimal_routes`  is checked (or true). 

The default value is `Preserve First` .

Values: `Preserve First`  \| `Preserve Last`  \| `Preserve First and Last`  \| `Preserve None` 
...
[return_to_start](/route-asynchronous-service#return_to_start)
...
Specify whether routes will start and end at the same location. 
...
[use_time_windows](/route-asynchronous-service#use_time_windows)
...
Specify whether the routes will use time windows specified on the input stops. 
...
[time_zone_for_time_windows](/route-asynchronous-service#time_zone_for_time_windows)
...
[time_of_day](/route-asynchronous-service#time_of_day)
...
[time_zone_for_time_of_day](/route-asynchronous-service#time_zone_for_time_of_day)
...
[uturn_at_junctions](/route-asynchronous-service#uturn_at_junctions)
...
[point_barriers](/route-asynchronous-service#point_barriers)
...
-   [As a collection of features](/route-asynchronous-service#syntax-examples-for-point_barriers)
-   [As a feature service URL](/route-asynchronous-service#syntax-examples-for-point_barriers)
...
[line_barriers](/route-asynchronous-service#line_barriers)
...
-   [As a collection of features](/route-asynchronous-service#syntax-examples-for-line_barriers)
-   [As a feature service URL](/route-asynchronous-service#syntax-examples-for-line_barriers)
...
[polygon_barriers](/route-asynchronous-service#polygon_barriers)
...
-   [As a collection of features](/route-asynchronous-service#syntax-examples-for-polygon_barriers)
-   [As a feature service URL](/route-asynchronous-service#syntax-examples-for-polygon_barriers)
...
[use_hierarchy](/route-asynchronous-service#use_hierarchy)
...
[restrictions](/route-asynchronous-service#restrictions)
...
Specify the restrictions that will be honored by the service. To learn about the accepted values for this parameter, see the [restrictions](/route-asynchronous-service#restrictions) section below.
...
[attribute_parameter_values](/route-asynchronous-service#attribute_parameter_values)
...
Specify additional values required by an attribute or restriction, such as whether the restriction prohibits, avoids, or prefers travel on restricted roads. To learn about the accepted values for this parameter, see the [attribute_parameter_values](/route-asynchronous-service#attribute_parameter_values) section below.
...
[route_shape](/route-asynchronous-service#route_shape)
...
[route_line_simplification_tolerance](/route-asynchronous-service#route_line_simplification_tolerance)
...
Specify how much the route geometry returned by the service will be simplified. You can specify the value as in the example below:
...
[populate_route_edges](/route-asynchronous-service#populate_route_edges)
...
Specify whether individual edges will be generated for each street along the route. 
...
[populate_directions](/route-asynchronous-service#populate_directions)
...
Specify whether the service will generate directions for each route. 
...
[directions_language](/route-asynchronous-service#directions_language)
...
Specify the language that will be used when generating directions. This parameter applies only when the `populate_directions`  parameter is set to `true` . To learn about the accepted values for this parameter, see the [directions_language](/route-asynchronous-service#directions_language) section below.
...
[directions_distance_units](/route-asynchronous-service#directions_distance_units)
...
Specify the units for displaying travel distance in the directions. This parameter applies only when the `populate_directions`  parameter is set to `true` . 
...
[directions_style_name](/route-asynchronous-service#directions_style_name)
...
Specify the name of the formatting style for the directions. This parameter applies only when the `populate_directions`  parameter is set to `true` . 
...
[impedance](/route-asynchronous-service#impedance)
...
[save_route_data](/route-asynchronous-service#save_route_data)
...
Specify whether the service will create a `.zip`  file that contains a file geodatabase containing the input and output values of the analysis in a format that can be used to share route layers with your portal. 
...
[save_output_network_analysis_layer](/route-asynchronous-service#save_output_network_analysis_layer)
...
[overrides](/route-asynchronous-service#overrides)
...
[time_impedance](/route-asynchronous-service#time_impedance)
...
[distance_impedance](/route-asynchronous-service#distance_impedance)
...
[output_format](/route-asynchronous-service#output_format)
...
[env:outSR](/route-asynchronous-service#envoutsr)
...
[ignore_invalid_locations](/route-asynchronous-service#ignore_invalid_locations)
...
[locate_settings](/route-asynchronous-service#locate_settings)
...
The service imposes a limit of 10,000 points that can be passed as stops. If this value is exceeded, the response returns an error message.

</Note>

The `stops`  parameter can be specified using a JSON structure that represents a set of point features. The property is optional; however, the JSON structure must specify either the `url`  or `features`  property: 
...
-   `RouteName` 
    ...
    The grouping is based entirely on the provided input and no attempt is made by the service to group stops in any optimal manner. To determine the optimal grouping of stops into routes, use the [fleet routing service](/vehicle-routing-problem-service) instead.

    </Note>

    You can group up to 150 stops into one route.
-   `Sequence` 
...
The following is the syntax for specifying stops using a JSON structure for features:
...
The units of the attribute value of `AdditionalTime`  and `AdditionalDistance`  on the [stops](/route-asynchronous-service#stops)  parameter are same as the units of the `measurement_units` .
...
When `reorder_stops_to_find_optimal_routes`  is `true` , you have options to preserve the starting or ending stops and let the tool reorder the rest. The first and last stops are determined by their sequence attribute values or, if the sequence values are null, by the order in which they are listed. 
...
-   `true` —The route should start and end at the first stop feature. When `reorder_stops_to_find_optimal_routes`  and `return_to_start`  are both true, `preserve_terminal_stops`  must be set to `Preserve First`  (which is the default value).
...
`https://logistics.arcgis.com/arcgis/rest/services/World/Route/GPServer/FindRoutes/jobs/<yourJobID>?token=<yourToken>&returnMessages=true&f=json` 
...
`https://logistics.arcgis.com/arcgis/rest/services/World/Route/GPServer/FindRoutes/jobs/<yourJobID>/cancel?token=<yourToken>&f=json` 
...
`https://logistics.arcgis.com/arcgis/rest/services/World/Route/GPServer/FindRoutes/jobs/<yourJobID>/results/<output_parameter_name>?token=<yourToken>&f=json` 

Instead of retrieving outputs individually, you can make a request of the following form to retrieve all the outputs.

`https://logistics.arcgis.com/arcgis/rest/services/World/Route/GPServer/FindRoutes/jobs/<yourJobID>/results?token=<yourToken>&f=json` 

Refer to the [Output parameters](/route-asynchronous-service#output-parameters) section for more information on how to interpret the solution provided by the service.
...
`https://logistics.arcgis.com/arcgis/rest/services/World/Route/GPServer/FindRoutes/jobs/<yourJobID>/results/<output_parameter_name>?token=<yourToken>&f=json&outSR=102100` 
...
`https://logistics.arcgis.com/arcgis/rest/services/World/Route/GPServer/FindRoutes/jobs/<yourJobID>/results?token=<yourToken>&f=json&outSR=102100` 

The below example shows how to retrieve the value for an input parameter by making the requests of the following form:

`https://logistics.arcgis.com/arcgis/rest/services/World/Route/GPServer/FindRoutes/jobs/<yourJobID>/inputs/<inputParameterName>?token=<yourToken>&f=json` 

The below example shows how to retrieve all the inputs by making the request of the following form.

`https://logistics.arcgis.com/arcgis/rest/services/World/Route/GPServer/FindRoutes/jobs/<yourJobID>/inputs?token=<yourToken>&f=json` 

## Output parameters

Upon successful execution, the service returns the best paths between the stops and the status indicating if the analysis was successful using the following output parameters. 
...
The `output_stops`  parameter is returned as a JSON feature set with following syntax:
...
-   A value of 0 (zero) indicates that the line begins at the from point of the underlying street feature.
-    A value of 1 indicates that the line begins at the to point of the street feature. 
...
-   A value of 0 (zero) indicates that the line ends at the from point of the underlying street feature.
-    A value of 1 indicates that the line ends at the to point of the street feature. 
...
The `output_route_edges`  parameter is returned as a JSON feature set with following syntax:

<Note type={"caution"}>

The service can only return a maximum of 1,000,000 features with the `output_route_edges`  parameter. If this limit is exceeded, the `exceededTransferLimit`  property is set to true. 

</Note>

```other
{
    "paramName": "Output_Routes_Edges",
    "dataType": "GPFeatureRecordSetLayer",
    "value": {
        "displayFieldName": "",
        "geometryType": "esriGeometryPolyline",
...
The table below lists the limits that apply to the job request route service. 
...
```other
https://logistics.arcgis.com/arcgis/rest/services/World/Route/GPServer/FindRoutes/submitJob?stops={"features":[{"attributes":{"Name":"Stop1"},"geometry":{"x":145.0657306480001,"y":-37.86513493399997}},{"attributes":{"Name":"Stop2"},"geometry":{"x":145.1049567670001,"y":-37.81866925199995}}]}&f=json&token=<yourToken>
```

 

```javascript
https://logistics.arcgis.com/arcgis/rest/services/World/Route/GPServer/FindRoutes/submitJob?stops=%7B%22features%22:%5B%7B%22attributes%22:%7B%22Name%22:%22Stop1%22%7D,
...
```other
https://logistics.arcgis.com/arcgis/rest/services/World/Route/GPServer/FindRoutes/jobs/<yourJobID>?returnMessages=true&f=json&token=<yourToken>
```
...
```other
https://logistics.arcgis.com/arcgis/rest/services/World/Route/GPServer/FindRoutes/jobs/<yourJobID>/results/output_stops?returnMessages=true&f=json&token=<yourToken>
```
...
```other
https://logistics.arcgis.com/arcgis/rest/services/World/Route/GPServer/FindRoutes/jobs/<yourJobID>/results/output_route_edges?returnMessages=true&f=json&<yourToken>
```
