...
{/* 
If the uploaded file is not in the [required format](/batch-geocode#upload-the-address-file), the response for the subsequent [jobs](/batch-geocode#check-the-job-status)  request will indicate that the job failed. */}
...
{/* 
If the field mapping passed in the [submitJob](/batch-geocode#submit-the-geocoding-job-to-the-service)  request includes a reference to a service input field name which does not exist, the response for the subsequent [jobs](/batch-geocode#check-the-job-status)  request will indicate that the job failed and identify the invalid field name. */}
...
### Invalid CSV field
{/* 
If the field mapping passed in the [submitJob](/batch-geocode#submit-the-geocoding-job-to-the-service)  request includes a reference to a nonexistent field name in the input CSV file, the response for the subsequent [jobs](/batch-geocode#check-the-job-status)  request will indicate that the job failed and identify the invalid field name. The error is also returned if a field name passed in the `fieldMapping`  parameter of the `submitJobs`  request uses different casing than the corresponding field name in the CSV file. For example, if the field name in the CSV file is `CITY` , but the name specified in the `fieldMapping`  is `City` , this error is returned. */}
...
{/* If there is a problem with the input CSV file's header, the response for the subsequent [jobs](/batch-geocode#check-the-job-status)  request will indicate that the job failed and return the following message. */}
