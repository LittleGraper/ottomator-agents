---
title: "Job request"
description: "Find directions using the global route service available with ArcGIS Enterprise."
slug: "route-async-services"
url: "/route-async-services"
guid: "GUID-3E10DA52-8F9B-4DD5-A4F2-B32A1A8A7E38"
migratedTopicMetadata:
  FTITLE: "Route async services"
  FDESCRIPTION: ""
  ...
  READ-ACCESS: ""
restInfoMetadata:
  apiPath:  /<serviceName>/GPServer/FindRoutes/submitJob
  urlSegments:
    - name: <root>
      url: /catalog
    - name:  /<serviceName>/GPServer/FindRoutes/submitJob
  methods: 
   - get
   - post
  sslOnly: 'no'
...
[stops](/route-async-services#stops) 
...
-   [As a collection of features](/route-async-services#stops)
-   [As a feature service URL](/route-async-services#stops)
...
[token](/route-async-services#token) 
...
[travel_mode](/route-async-services#travel_mode) 
...
[measurement_units](/route-async-services#measurement_units) 
...
[analysis_region](/route-async-services#analysis_region) 
...
[reorder_stops_to_find_optimal_routes](/route-async-services#reorder_stops_to_find_optimal_routes) 
...
[preserve_terminal_stops](/route-async-services#preserve_terminal_stops) 
...
[return_to_start](/route-async-services#return_to_start) 
...
[use_time_windows](/route-async-services#use_time_windows) 
...
[time_zone_for_time_windows](/route-async-services#time_zone_for_time_windows) 
...
[time_of_day](/route-async-services#time_of_day) 
...
[time_zone_for_time_of_day](/route-async-services#time_zone_for_time_of_day) 
...
[uturn_at_junctions](/route-async-services#uturn_at_junctions) 
...
[point_barriers](/route-async-services#point_barriers) 
...
-   [As a collection of features](/route-async-services#syntax-examples-for-point_barriers)
-   [As a feature service URL](/route-async-services#point_barriers)
...
[line_barriers](/route-async-services#line_barriers) 
...
-   [As a collection of features](/route-async-services#syntax-examples-for-line_barriers)
-   [As a feature service URL](/route-async-services#line_barriers)
...
[polygon_barriers](/route-async-services#polygon_barriers) 
...
-   [As a collection of features](/route-async-services#polygon_barriers)
-   [As a feature service URL](/route-async-services#polygon_barriers)
...
[use_hierarchy](/route-async-services#use_hierarchy) 
...
[restrictions](/route-async-services#restrictions) 
...
Specify the restrictions that will be honored by the service. To learn about the accepted values for this parameter, see the [restrictions](/route-async-services#restrictions) section below.
...
[attribute_parameter_values](/route-async-services#attribute_parameter_values) 
...
Specify additional values required by an attribute or restriction, such as whether the restriction prohibits, avoids, or prefers travel on restricted roads. To learn about the accepted values for this parameter, see the [attribute_parameter_values](/route-async-services#attribute_parameter_values) section below.
...
[route_shape](/route-async-services#route_shape) 
...
[route_line_simplification_tolerance](/route-async-services#route_line_simplification_tolerance) 
...
[populate_route_edges](/route-async-services#populate_route_edges-) 
...
[populate_directions](/route-async-services#populate_directions) 
...
[directions_language](/route-async-services#directions_language) 
...
Specify the language that will be used when generating directions. This parameter applies only when the `populate_directions`  parameter is set to `true` . To learn about the accepted values for this parameter, see the [directions_language](/route-async-services#directions_language) section below.
...
[directions_distance_units](/route-async-services#directions_distance_units) 
...
[directions_style_name](/route-async-services#directions_style_name) 
...
[impedance](/route-async-services#impedance) 
...
[save_route_data](/route-async-services#save_route_data) 
...
[save_output_network_analysis_layer](/route-async-services#save_output_network_analysis_layer) 
...
[overrides](/route-async-services#overrides) 
...
[time_impedance](/route-async-services#time_impedance) 
...
[distance_impedance](/route-async-services#distance_impedance) 
...
[output_format](/route-async-services#output_format) 
...
[env:outSR](/route-async-services#envoutsr) 
...
[ignore_network_location_fields](/route-async-services#ignore_network_location_fields) 
...
[ignore_invalid_locations](/route-async-services#ignore_invalid_locations) 
...
[locate_settings](/route-async-services#locate_settings) 
...
[accumulate_attributes](/route-async-services#accumulate_attributes) 
...
The `ObjectID`  of the feature in the `RouteJunctions`  feature class that is coincident with the beginning of the `RouteEdges`  feature.
...
The `ObjectID`  of the feature in the `RouteJunctions`  feature class that is coincident with the end of the `RouteEdges`  feature.
...
```javascript
https:///FindRoutes/submitJob?parameters
```

 

The route service supports synchronous and asynchronous modes. [Synchronous and asynchronous modes](/an-overview-of-routing-services/#request-types) define how the application interacts with the service and gets the results. 

The request URL and the parameter names supported by the service when using synchronous operation are different and are described in the [Route service with synchronous operation](/route-sync-services) topic.

<Note type={"caution"}>

The maximum time an application can use the route service when using the asynchronous mode is 1 hour (3,600 seconds) by default. If your request does not complete within this time limit, it will time out and fail. You can increase this time-out by modifying the **The maximum time a client can use a service** property using ArcGIS Server Manager
...
```other
https:///FindRoutes/submitJob?stops={"features":[{"attributes":{"Name":"Stop1"},"geometry":{"x":145.0657306480001,"y":-37.86513493399997}},{"attributes":{"Name":"Stop2"},"geometry":{"x":145.1049567670001,"y":-37.81866925199995}}]}&f=json&token=<yourToken>
```

 

```javascript
https:///FindRoutes/submitJob?stops=%7B%22features%22:%5B%7B%22attributes%22:%7B%22Name%22:%22Stop1%22%7D,
...
```other
https:///FindRoutes/jobs/<yourJobID>?returnMessages=true&f=json&token=<yourToken>
```
...
```other
https:///FindRoutes/jobs/<yourJobID>/results/output_stops?returnMessages=true&f=json&token=<yourToken>
```
...
```other
https:///FindRoutes/jobs/<yourJobID>/results/output_route_edges?returnMessages=true&f=json&<yourToken>
```
...
```javascript
https:///FindRoutes/jobs/<yourJobID>?token=<yourToken>&returnMessages=true&f=json
```
...
```javascript
https:///FindRoutes/jobs/<yourJobID>/cancel?token=<yourToken>&f=json
```
...
```javascript
https:///FindRoutes/jobs/<yourJobID>/results/<output_parameter_name>?token=<yourToken>&f=json
```
...
```javascript
https:///FindRoutes/jobs/<yourJobID>/results/<output_parameter_name>?token=<yourToken>&f=json&outSR=102100
```
...
```javascript
https:///FindRoutes/jobs/<yourJobID>/inputs/<inputParameterName>?token=<yourToken>&f=json
```
...
Maximum time a client can use the asynchronous route service.
