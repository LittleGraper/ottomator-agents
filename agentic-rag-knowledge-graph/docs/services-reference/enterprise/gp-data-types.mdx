---
title: "Geoprocessing service data types"
uid: "gp-data-types"
description: "The majority of parameters that you include in a request are dependent on the input parameter types of the geoprocessing task you are running."
slug: "gp-data-types"
url: "/gp-data-types"
guid: "GUID-BBB0614E-11D2-4FA2-82F6-EC8E8E104093"
migratedTopicMetadata:
  FTITLE: "GP Data Types"
  FDESCRIPTION: "One stop place for all supported data types for geoprocessing services."
  FCHANGES: "new GPLong and date fields, and composite"
  FISHRELEASELABEL: ""
  FESRISOFTWARERELEASE: ""
  FESRITAGCLOUD: ""
  FESRIRECREATIONPROCEDURE: ""
  FESRIPRODUCTLIFECYCLE: "11.2 - *"
  CREATED-ON: "03/07/2023 12:27:00"
  FAUTHOR: "ssuo"
  FSTATUS: "Released"
  MODIFIED-ON: "04/10/2023 13:24:04"
  VERSION: "8"
  FRESOLUTION: ""
  DOC-LANGUAGE: "en"
  FISHREVCOUNTER: "18"
  ED: "GUID-F86D0085-2880-4963-B0EF-F8AE3413336E"
  FUSERGROUP: "VUSERGROUPARCGISONLINEDOC"
  READ-ACCESS: ""
restInfoMetadata: {}
---

The majority of the parameters that you include in a request are dependent on the input parameter types of the geoprocessing task you are submitting. The geoprocessing [`execute`](/execute-gp-task) and [`submitJob`](/submit-gp-job) operations share the same syntax for all input parameter types. For outputs, the result data syntax may be different from the inputs.

The data types listed in this topic are the data types supported on the server side. To learn more about supported data types from the publishing client, and how unsupported data types are converted during the publish process, see [Input and output parameters](https://pro.arcgis.com/en/pro-app/latest/help/sharing/overview/inputs-and-outputs.htm). There is also an identical JSON Schema page availalbe at all the [GPServer](/gp-service/) endpoint describing all data types.
...
For the syntax of `featureSet`, see [featureSet object](/featureset-object/).

#### Layer syntax

For the syntax of a layer, see [Layer object](/layer/).

#### Layer URL syntax

To specify a feature service layer or map service layer, provide the URL to the layer. An optional attribute filter can be added. Starting at 10.7.1, instead of querying [time-enabled map service layers](/query-map-service-layer#request-parameters) or [time-enabled feature service layers](/query-feature-service-layer#request-parameters), an optional time filter can be specified. Time can be filtered as a single integer timestamp using epoch time. Time can also be a range in a comma-delimited string, with a lower end and an upper end. Either end of the range can be `null`.
...
For a large set of geometries, you can specify an uploaded item as input for geoprocessing services that support [uploads](/uploads), which is one of the **Child Resources** values of the geoprocessing service. The item ID will be available once you use the [`upload` item](DEVSITE/rest/enterprise-administration/server/uploaditem) operation to upload a JSON-based `.txt` file or a `.json` file. The `itemID` value of the portal item is not supported regardless of the type of item, even if the server is federated.
...
When using a script tool or [Python Toolbox](https://pro.arcgis.com/en/pro-app/latest/arcpy/geoprocessing_and_python/accessing-parameters-within-a-python-toolbox.htm), use the [`arcpy.GetParameter`](https://pro.arcgis.com/en/pro-app/latest/arcpy/functions/getparameter.htm) or `parameters[<n>].value`, instead of [`arcpy.GetParameterAsText`](https://pro.arcgis.com/en/pro-app/latest/arcpy/functions/getparameterastext.htm) or `parameters[<n>].valueAsText`.
...
If the geoprocessing service is not associated with a result map service, and `returnFeatureCollection` is set to `true`, the parameter value for `GPFeatureRecordSetLayer` is a JSON structure similar to the input [`layer`](/gp-data-types#layer-syntax) object.
...
The output can be a feature service. See the [Feature service output](/gp-data-types#feature-service-output) section above.
...
For geoprocessing services that support [`uploads`](/uploads), you can specify an uploaded item as input to `GPRasterDataLayer` as follows:
...
```json
{
  ...
  "dataType": "GPRasterDataLayer",
  "value": {
    "url": "https://machine.domain.com/webadaptor/rest/services/prefix_Output_Raster_Layer/ImageServer",
    "type": "ImageService"
  }
}
```
...
For geoprocessing services that support [`uploads`](/uploads), you can specify an [uploaded](/upload) item as input to `GPDataFile` as follows. The `uploads` operation will upload an item into ArcGIS Server resulting an `itemID`. However, your service can use other ArcGIS Server items as long as you provide a correct `itemID`.
...
Starting at ArcGIS Enterprise 10.8, you can specify a portal [itemID](DEVSITE/rest/users-groups-and-items/item/) as an input when you publish geoprocessing services to a federated server. You can not use a portal item ID if your geoprocessing service is not in the same portal as that item.
...
By default, the array of arrays syntax will be the output for `GPValueTable`. To retrieve the column names along with all values, use `returnColumnName=True` for operations related to [`execute`](/execute-gp-task) or [`GPResult`](/gp-result).
...
To provide a spatial refernece or coordinate system based on its wkid, latest wkid, vertical coordinate system id, use the following syntax.
...
```json
{
    "name": "Sinusodial (World)",
    "vcsName": "MSL Height"
}
```

#### Projection file syntax
If you have a projection file with a prj file extention, you can use the file directly, or use the [upload](/upload) operation of a geoprocessing service to upload that file.
```json
{
    "projectionFile": "https://machine.domain.com/myprojection.prj"
}
```
```json
{
...
Both data types will require you to provide the `xmin`, `ymin`, `xmax`, `ymax` values. If you want to optionally provide `zmin`, `zmax`, `mmin`, `mmax`, they are supported as well. Spatail reference is required for `GPExtent`, although the only acceptted format of the spatial reference is by providing any of the `wkid`, `latestWkid`, `vcsWkid`, `latestVcsWkid`, `wkt`, and `wkt2`. You cannot use the name of a spatial reference, nor a spatial refernece projection file to specify the spatial reference of an extent. If you don't specify the spatial reference of `GPExtent`, the default spatial reference will apply.
...
### Output
Although using either the GPExtent or GPEnvelope is rare as an output, they are still avaialbe, and the syntax is the same as the input.

## GPStringHidden
Supported at ArcGIS Enterprise 11.4 or later, if you publish your tool containing the `string hidden` data type from ArcGIS Pro 3.4 or later, you will see these data types will become `GPStringHidden` by default in a geoprocessing service. The string is not encrypted. The actual value will be visible through other REST resources, like the [input](/gp-input) and [inputs](/inputs) resources, if applicable. It is only used by client applications to render the display of this data type differently than a regular `GPString`. Most commonly, a client application will render the string as `*` characters. It is not possible to have a default value of a data type with `GPStringHidden`.
