---
title: "ExportWebMap specification"
description: "The ExportWebMap specification defines the state of a web map using JavaScript Object Notation (JSON). The specification contains layer and extent information necessary for creating a printable image of the map."
slug: "exportwebmap-specification"
url: "/exportwebmap-specification"
guid: "GUID-0E73F217-FBA8-4873-87C1-7423EAB77F91"
migratedTopicMetadata:
  FTITLE: "ExportWebMap specification"
  FDESCRIPTION: ""
  FCHANGES: ""
  FISHRELEASELABEL: ""
  ...
  CREATED-ON: "22/06/2023 15:13:14"
  FAUTHOR: "thoque"
  FSTATUS: "Released"
  MODIFIED-ON: "28/09/2023 12:30:05"
  VERSION: "20"
  FRESOLUTION: ""
  DOC-LANGUAGE: "en"
  FISHREVCOUNTER: "6"
  ED: "GUID-C0C7A32F-C8CF-4E31-AF02-9DAC51E2BC89"
...
- Added support to work with dynamic text element, table frame element and chart frame element in a layout. You need to use `elementOverrides` inside `layoutOptions` for that.
    Notes: You can use a print service `Get Layout Templates Info` task to find out if a layout template has any of these types of elements.
...
- Added support to turn visibility on or off for a north arrow element in a layout. You need to use `elementOverrides` inside `layoutOptions` for that.
    Note: You can use a print service `GetLayoutTemplateInfos` task to find out if a layout template has a north arrow.
...
<StyledTable headers={"Syntax:,Example"}>
...
For a map service, feature service, or image service, use a URL that points to the REST endpoint of the service. SOAP endpoints are not supported.

</Note>

<StyledTable headers={"Syntax for operational layers"}>
...
<StyledTable headers={"Syntax for a map service layer,Example"}>
...
-   `source`  (optional): Represents the source of a layer that gets added to the map service dynamically per request. See the [ArcGIS REST API](http://links.esri.com/arcgis/rest/api) for more information on how to define source.
...
<StyledTable headers={"Syntax for a feature layer,Example"}>
...
<StyledTable headers={"Syntax for an image service layer,Example"}>
...
See the [ArcGIS REST API](http://links.esri.com/arcgis/rest/api) for more information on image service layers.
...
<StyledTable headers={"Syntax for a WMS service layer"}>
...
<StyledTable headers={"Syntax for a KML layer,Example"}>
...
<StyledTable headers={"Syntax for a client-side image,Example"}>
...
<StyledTable headers={"Syntax for client side graphics,Example"}>
...
<StyledTable headers={"Syntax for Comma-Separated Values (CSV) file by URL,Example"}>
...
See the [ArcGIS REST API](http://links.esri.com/arcgis/rest/api) for more information on the following properties:
...
<StyledTable headers={"Syntax to define  for a temporal renderer,Example"}>
...
<StyledTable headers={"Syntax for baseMap,Example"}>
...
<StyledTable headers={"Syntax for Vector Tile Layer as,Example"}>
...
<StyledTable headers={"Syntax for Bing Maps as a,Example"}>
...
<StyledTable headers={"Syntax for  as a,Example"}>
...
<StyledTable headers={"Syntax for WMTS as a"}>
...
<StyledTable headers={"ExportWebMap custom text element,Corresponding dynamic text in"}>
...
"layoutOptions": {
  ...
  "legendOptions": {
    "operationalLayers": [
      {
        "id": "<webmapOperationalLayerId>",
        "dynamicLegend": true|false,   //for feature layer or image layer
        "sublayerIds": [  //array of string or number
          <mapServerSublayerId1>,
          <mapServerSublayerId2>
        ],
        "subLayerDynamicLegends": [true|false, true|false, ...]   //for map image layer sub layers.
      }
    ]
  }
}
  ...
  "legendOptions": {
    "operationalLayers": [
      { //for dynamic mapservicelayer
        ...
        "subLayerDynamicLegends": [true, false]
      },
      { //for WMS layer
        "id": "myWMSLayer",
...
-   `legendOptions`  (optional): Specifies properties of a legend element on the layout. Please note that when a legend element on a layout is found empty (it does not have layer in it), it gets removed by the printing service.
    ...
    -   `operationalLayers`  (optional): Specifies the operational layers whose legends will be added to the layout.
        ...
        -   `dynamicLegend` (optional): enable or disable dyanamic legend option for a feature layer or an image layer.
        ...
        -   `subLayerDynamicLegends` (optional): And array of booleans where the order matches the layer id order in `subLayerIds` to indicate if the dynamic-legends is applied to a layer or not.
