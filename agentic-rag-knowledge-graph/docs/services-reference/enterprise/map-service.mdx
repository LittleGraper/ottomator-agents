---
title: "Map Service"
description: "Map services offer access to the contents of a map hosted on a server."
slug: "map-service"
url: "/map-service"
guid: "GUID-EFAA6DC2-DC48-483C-871C-CD956BE4797C"
migratedTopicMetadata:
  ...
  CREATED-ON: "20/09/2023 16:27:28"
  FAUTHOR: "thoque"
  FSTATUS: "Released"
  MODIFIED-ON: "02/10/2023 07:56:17"
  VERSION: "21"
  FRESOLUTION: ""
  DOC-LANGUAGE: "en"
  FISHREVCOUNTER: "5"
  ED: "GUID-C69A50E5-78A5-4F07-A040-C60D05FB1AED"
  FUSERGROUP: "VUSERGROUPARCGISONLINEDOC"
  READ-ACCESS: ""
restInfoMetadata:
  apiPath: /<serviceName>/MapServer
  urlSegments:
    - name: <catalog-url>
      url: /catalog
    - name: /<serviceName>/MapServer
  methods: []
  sslOnly: 'no'
...
Map services hosted by ArcGIS Online or Portal for ArcGIS can only return tiles from the server's cache; they cannot draw images dynamically, nor do they allow query of the individual features behind the map. You can support queries and informational pop-up windows in your applications using [feature services](/feature-service) in conjunction with your map services.
...
-   [ Export map](/export-map)—Exports a map image from a dynamic map service. The resulting map can be used for display and be in a different projection from the original data source. When generating a map image, map services cannot change feature rendering for an existing layer, add a dynamic layer, or change the layer draw order. 
-   [ Identify](/identify-map-service)—Returns information about features in one or more layers based on where a user clicks on the map.
-   [ Find](/find)—Returns information about features in one or more fields in one or more layers based on a keyword.
-   [ Generate KML](/generate-kml)—Generates a KML document wrapped in a KMZ file. The document contains a network link to the KML service endpoint with specified properties and parameters. This operation is valid on services that have not been restricted using a token service. 
-   [ Query](/query-map-service-layer) on a [ Layer/Table](/layer-table)—Returns a subset of features in a layer or records in a table based on query criteria.
...
-   `documentInfo.Version`  indicates the version of original source map document. You will only see this for map service that are running on ArcGIS Pro based runtime. "10.1" means that the originally the service was published from ArcMap. Any other values reflects an ArcGIS Pro version.
    ...
    With the release of ArcGIS Enterprise 11.0, support for the ArcMap service runtime has been deprecated.
...
-   Supports the following new parameters in `export` , `identify` , `find`  and `queryLegends`  operations on map services published from ArcGIS Pro:
    ...
    -   `spatialFilter`  to draw or query only features that meet the spatial filter criteria.
        ...
        Before using the `clipping`  or `spatialFilter`  parameters, ensure that the map service's root resource includes the `supportsClipping`  and `supportsSpatialFilter`  properties, respectively.
...
-   The `maxLOD`  is the maximum level of detail (LOD) of the tiling scheme at which the cache tiles are available in the tile service. Any requests to draw tiles outside this level would try to use the resampling setting to resample tiles from the nearest available LOD. The `maxLOD`  should always be larger than the `minLOD` .
    ...
    The `minLOD`  and `maxLOD`  values are validated automatically when publishing to ArcGIS Server, Portal for ArcGIS, and ArcGIS Online environments. To edit these values, you will need to use the [Edit Service](DEVSITE/rest/enterprise-administration/server/editservice) operation.
...
-   Added support for Layer File (`.lyrx` ) as a dynamic layer source: [Dynamic Workspace Layer](/layer-source-object/#dynamic-workspace-layer) .
...
-   GeoJSON is supported as an output format of the map service layer [query](/query-map-service-layer) operation. 
...
-   Query operations on [ Layer](/query-map-service-layer) and [ Dynamic Layer](/query-map-service-dynamic-layer) support a new parameter called `returnDistinctValues`  that accepts a Boolean value. When true, query results will contain distinct values based on the fields specified in the `outFields`  parameter.
-   Query operations on [ Layer](/query-map-service-layer) and [ Dynamic Layer](/query-map-service-dynamic-layer) support the `geometry`  parameter to be used in conjunction with the `outStatistics`  parameter. 
-   The [ Legend](/legend-map-service) resource returns the height and width of the legend symbol. 
-   The [ All Layers/Table](/all-layers-and-tables), [ Layer](/layer-table), and [ Dynamic Layer](/dynamic-layer-table) resources now include an `ownershipBasedAccessControlForFeatures`  flag to indicate ownership. 
